...
def read_raw_antcnt(input_fname, montage=None, eog=(), event_id=None, start_n_times=0, stop_n_times=-1,
    ...
    return RawANTCNT(input_fname=input_fname, montage=montage, preload=preload, 
                     ...
                     verbose=verbose, start_n_times=start_n_times, stop_n_times=stop_n_times)
...
class RawANTCNT(BaseRaw):
    ...
    def __init__(self, input_fname, montage, eog=(), event_id=None,
                 event_id_func='strip_to_integer', preload=False,
                 verbose=None, start_n_times=0, stop_n_times=-1):
        # print ('6') # second
        """Read ANT .cnt file.
        """
        ...
        # trig = eeg.get_trigger_count()
        # [print (eeg.get_trigger(i)[0].split(',')[-1].split(' ')) for i in range(eeg.get_trigger_count())]
        [print (eeg.get_trigger(i)) for i in range(eeg.get_trigger_count())]
        # print (eeg.get_trigger_count())
        # impedance = eeg.get_trigger(0)[-1].split(' ')
        # print (len(impedance))

        if stop_n_times != -1:
            n_times = stop_n_times

        # print (len(eeg.get_samples(0,1))) # 152

        # print (n_chan, n_times)

        data = np.empty((n_chan+1, n_times-start_n_times), dtype=np.double)
        from numpy import asarray
        x = asarray(eeg.get_samples(start_n_times,n_times))
        x.shape  = (n_times-start_n_times,n_chan) 
        
        data[:-1] = x.transpose()
        
        
        data *= CAL
        data[-1] = self._event_ch[start_n_times:n_times]
