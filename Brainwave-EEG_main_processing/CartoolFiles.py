...
class Bdf:
    def __init__(self,BdfFile,InfoOnly=False):
        ...
        if os.path.isfile(H5Name):
            OutPutFile=tables.openFile(BdfFile.replace('.bdf','.h5'),'r')
            ChanelName=OutPutFile.getNode('/ChanelName').read()
            ...
            if InfoOnly:
                ...
                OutPutFile=tables.openFile(BdfFile.replace('.bdf','.h5'),'w')
                OutPutFile.createArray('/','NbChanel',NbChanel)
                OutPutFile.createArray('/','ChanelName',ChanelName)
                OutPutFile.createArray('/','FS',FS)
                RawData=OutPutFile.createEArray('/','RawData',tables.Float64Atom(),(NbChanel,0))
                AvgRef=OutPutFile.createEArray('/','AvgRef',tables.Float64Atom(),(NbChanel,0))
                ...
                if trig!=[]:
                    self.Statut=OutPutFile.getNode('/RawData')[trig,:]
                OutPutFile.close()   
